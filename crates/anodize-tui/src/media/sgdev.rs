...
impl SgDev {
    ...
    pub fn drive_status(&self) -> Result<i32> {
        ...
        let s = unsafe { cdrom_drive_status_raw(fd) }
            .context("CDROM_DRIVE_STATUS ioctl")?;
        Ok(s)
    }
    ...
    pub fn cdb_in(&self, cdb: &[u8], buf: &mut [u8], timeout_ms: u32) -> Result<usize> {
        self.send(cdb, SG_DXFER_FROM_DEV,
    ...
    pub fn cdb_out(&self, cdb: &[u8], data: &[u8], timeout_ms: u32) -> Result<()> {
        self.send(cdb, SG_DXFER_TO_DEV,
                  data.as_ptr() as *mut _, data.len() as u32,
                  timeout_ms)?;
        Ok(())
    }
